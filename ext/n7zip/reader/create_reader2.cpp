...
buildFdInStreamArg(Napi::Object obj)
{
  ...
  if (!v_source.IsNumber()) {
    return type_err<InStreamArg>("The source of fd type stream must be Number");
  }
...
buildPathInSteamArg(Napi::Object obj)
{
  ...
  if (!v_source.IsString()) {
    return type_err<InStreamArg>("The source of path type stream must be String");
  }
...
buildBufferStreamArg(Napi::Object obj)
{
  ...
  if (!v_source.IsBuffer()) {
    return type_err<InStreamArg>("The source of buffer type stream must be Buffer");
  }
  ...
  if (ShareBuffer) {
    ...
    return ok(new InStreamArg(std::make_unique<char[]>(buf.Length())));
  }
}
...
buildMultiStreamArg(Napi::Object obj)
{
  auto v_source = obj.Get("source");
  if (!v_source.IsArray()) {
    return type_err<InStreamArg>("The source of multi type stream must be Array");
  }
...
buildInStreamArg(Napi::Object obj, bool in_streams)
{
  ...
  if (type == "fd") {
    ...
    if (in_streams) {
      ...
      return type_err<InStreamArg>("Multi-stream in multi-stream is not supported");
    }
  } else {
